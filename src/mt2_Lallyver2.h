...
// implementation, but as 2*B, 2*D and 2*E as per Cheng & Han - makes no difference, purely down to convention used.
    ...
    const double Fqi = mnbsq + pmissx*pmissx + pmissy*pmissy -
                ((mnasq + masq - mnbsq - mbsq)/(2.*Eb) + (pbx*pmissx + pby*pmissy)/Eb)
                *((mnasq + masq - mnbsq - mbsq)/(2.*Eb) + (pbx*pmissx + pby*pmissy)/Eb); //constant term ie *delta^0
...
// The coefficients for the eight-order polynomial are as follows (skip if you are of a sensitive nature!):
    ...
    double disc6 = 18*(a2*b2*(c2*d0 + c1*d1 + c0*d2) + c2*d2*(a2*b0 + a1*b1 + a0*b2) + (a2*b1 + b2*a1)*(c2*d1 + d2*c1)) -
                ...
                4*b2*b1*c2*c1 + b2*b2*c1*c1 + c2*c2*b1*b1 - 4*(a2*(3*c2*c2*c0 + 3*c2*c1*c1) + 3*a1*c2*c2*c1 +
                a0*c2*c2*c2) - 54*(a2*a2*d2*d0 + d2*d2*a2*a0 +2*a2*a1*d2*d1) - 27*(a2*a2*d1*d1 + a1*a1*d2*d2); //sixth-order term of delta
    double disc5 = 18*(a2*b2*(c1*d0 + c0*d1) + c2*d2*(a1*b0 + a0*b1) + (a2*b1 + b2*a1)*(c2*d0 + c1*d1 + c0*d2) +
                (c2*d1 + d2*c1)*(a2*b0 + a1*b1 + a0*b2)) - 4*(3*b2*b2*b1*d0 + d1*(3*b2*b2*b0 + 3*b2*b1*b1) +
                d2*(6*b2*b1*b0 + b1*b1*b1)) + 2*b2*b2*c1*c0 + 2*b1*b0*c2*c2 + 4*b2*b1*c2*c0 + 4*c2*c1*b2*b0 +
    ...
    double disc4 = 18*(a2*b2*c0*d0 + a0*b0*c2*d2 + (a2*b0 + a1*b1 + a0*b2)*(c2*d0 + c1*d1 + c0*d2) + (a2*b1 + a1*b2)*(c1*d0 + c0*d1) +
                (a1*b0 + a0*b1)*(c1*d2 + c2*d1)) - 4*(d0*(3*b2*b2*b0 + 3*b2*b1*b1) + d1*(6*b2*b1*b0 + b1*b1*b1) +
                d2*(3*b2*b0*b0 + 3*b1*b1*b0)) + b2*b2*c0*c0 + c2*c2*b0*b0 + 4*b2*b1*c1*c0 + 4*b1*b0*c2*c1 + 4*b2*b0*c2*c0 +
                2*b2*b0*c1*c1 + 2*c2*c0*b1*b1 + b1*b1*c1*c1 -4*(a0*(3*c2*c2*c0 + 3*c2*c1*c1) + a1*(6*c2*c1*c0 + c1*c1*c1) +
                a2*(3*c2*c0*c0 + 3*c0*c1*c1)) - 27*(a2*a2*d0*d0 + d2*d2*a0*a0 + 4*a2*a1*d1*d0 + 4*a1*a0*d2*d1 +
                4*a2*a0*d2*d0 + a1*a1*d1*d1 +2*a2*a0*d1*d1 + 2*a1*a1*d2*d0); //fourth-order term of delta
    double disc3 = 18*(c0*d0*(a2*b1 + a1*b2) + a0*b0*(c2*d1 + c1*d2) + (a2*b0 + a1*b1 + a0*b2)*(c1*d0 + c0*d1) +
                (a1*b0 + a0*b1)*(c2*d0 + c1*d1 + c0*d2)) - 4*(d0*(6*b2*b1*b0 + b1*b1*b1) + d1*(3*b2*b0*b0 + 3*b1*b1*b0) +
                3*b1*b0*b0*d2) + 2*b2*b1*c0*c0 + 2*b0*b0*c2*c1 + 4*b2*b0*c1*c0 + 4*b1*b0*c2*c0 + 2*c1*c0*b1*b1 + 2*b1*b0*c1*c1 -
...
// Next we need to check for unbalanced events, we will use different methods depending on elliptic or parabolic (ie massless) situations
    ...
    if(delta0 != delta0) { // in case delta0 is undefined/infinite - can happen if input data is suspect i.e. side a momenta and mass (and thus Ea) all = 0
        delta0 = 0.0;
    }
...
// (note intersection of ellipses methodology used is that due to Walker, J.W. see arXiv 1311:6219 for more details)
    ...
    if(deltaIntersect != deltaIntersect) { // in case deltaIntersect is undefined/infinite - can happen if input data is suspect i.e. zero transverse momenta
        deltaIntersect = 0.0;
    }
	...
	if ((ma + mb + mna + mnb) < 0.01) { // to capture "quasi-unbalanced" cases as well - these are likely to be very small MT2 values
...
*/
	...
	if((nooflambdasgnchanges > 1) || (delta == deltaMax)) { // last condition ensures a new root is searched for if Newton iteration did not find a valid root
    ...
    if(MT2 != MT2) { // in case MT2 is undefined/infinite - would only happen if input data is suspect i.e. all zero values etc.
        MT2 = 0.0;
    }
    return MT2;
}
...
//                // Divide by zero error shouldn't happen - will have to resort to Regula Falsi method
            ...
            if(!(outsideLB && (x1 < LB)) && !(k > maxIterations) && !(outsideUB && (x1 > UB)) ) { // Check if Newton's method still OK i.e. is staying within the original bounds and not heading towards a different root
                if( x1 < LB ) {
		...
		if((stuckInLoopArray[0] == stuckInLoopArray[4]) || (stuckInLoopArray[0] == stuckInLoopArray[3]) || (stuckInLoopArray[0] == stuckInLoopArray[2])) {
            if((stuckInLoopArray[2] == stuckInLoopArray[0]) && (stuckInLoopArray[4] != stuckInLoopArray[0])) {
                ...
                if((((fabs(x1 - xNR)/fabs(x1) < accuracy) || (fabs(y_Newt/yprime) < accuracy)) && (k>2) ) || (stuckInLoop && (stuckInLoopArray[4] != 99.0))){ // if the result has met the desired precision && just doublecheck is a root!
                    solutionFound = true;
                    if(stuckInLoop) { //just take the largest of last five values as a new upper bound before returning to main
...
// We will search for a root that will leave the lambda function with only one positive root
	...
	double l_nooflambdasgnchanges = 2;
   	...
   	while(sgnLoop <= bisectMaxLoops) { //we will reduce the interval until no. of lambda function sgn changes is 1 (or zero) and then we have both a new LB and the previous value is a new UB
		...
		if((sgnLoop > 1) && (sgnLoop<(bisectMaxLoops+1))) { // if we have bisected this number of times we are at machine precision, we are at the correct delta value
...
// Now we need to check that deltaMaxNew is the new LB ie f(LB)*f(deltaMaxNew) should be > 0 (else it's actually a new UB)
            ...
            if(rootBounds > 0) { // we have the new LB, now need to find the new UB
...
//Need to transpose function candidate UB to check there is only one root below the candidate UB value and above the candidate LB (awkward check but couldn't think of more efficient way)
                    ...
                    const double transDdelta = (discPolynomial.Coeffs5 + 56*checkdeltasq*checkdelta*discPolynomial.Coeffs8 + 21*checkdeltasq*discPolynomial.Coeffs7
                                        + 6*checkdelta*discPolynomial.Coeffs6);
                    const double transEdelta = (discPolynomial.Coeffs4 + 70*checkdeltasq*checkdeltasq*discPolynomial.Coeffs8 + 35*checkdeltasq*checkdelta*discPolynomial.Coeffs7
                                        + 15*checkdeltasq*discPolynomial.Coeffs6 +5*checkdelta*discPolynomial.Coeffs5);
                    const double transFdelta = (discPolynomial.Coeffs3 + 56*checkdeltasq*checkdeltasq*checkdelta*discPolynomial.Coeffs8 + 35*checkdeltasq*checkdeltasq*discPolynomial.Coeffs7
                                        + 20*checkdeltasq*checkdelta*discPolynomial.Coeffs6 + 10*checkdeltasq*discPolynomial.Coeffs5 + 4*checkdelta*discPolynomial.Coeffs4);
                    const double transGdelta = (discPolynomial.Coeffs2 + 28*checkdeltasq*checkdeltasq*checkdeltasq*discPolynomial.Coeffs8 + 21*checkdeltasq*checkdeltasq*checkdelta*discPolynomial.Coeffs7
                                        + 15*checkdeltasq*checkdeltasq*discPolynomial.Coeffs6 + 10*checkdeltasq*checkdelta*discPolynomial.Coeffs5 + 6*checkdeltasq*discPolynomial.Coeffs4 + 3*checkdelta*discPolynomial.Coeffs3);
                    const double transHdelta = (discPolynomial.Coeffs1 + 8*checkdeltasq*checkdeltasq*checkdeltasq*checkdelta*discPolynomial.Coeffs8 + 7*checkdeltasq*checkdeltasq*checkdeltasq*discPolynomial.Coeffs7
                                        + 6*checkdeltasq*checkdeltasq*checkdelta*discPolynomial.Coeffs6 + 5*checkdeltasq*checkdeltasq*discPolynomial.Coeffs5 + 4*checkdeltasq*checkdelta*discPolynomial.Coeffs4
                                        + 3*checkdeltasq*discPolynomial.Coeffs3 + 2*checkdelta*discPolynomial.Coeffs2);
                    const double transIdelta = (discPolynomial.Coeffs0 + checkdeltasq*checkdeltasq*checkdeltasq*checkdeltasq*discPolynomial.Coeffs8 + checkdeltasq*checkdeltasq*checkdeltasq*checkdelta*discPolynomial.Coeffs7
                                        + checkdeltasq*checkdeltasq*checkdeltasq*discPolynomial.Coeffs6 + checkdeltasq*checkdeltasq*checkdelta*discPolynomial.Coeffs5 + checkdeltasq*checkdeltasq*discPolynomial.Coeffs4
                                        + checkdeltasq*checkdelta*discPolynomial.Coeffs3 + checkdeltasq*discPolynomial.Coeffs2 + checkdelta*discPolynomial.Coeffs1);
                    int nscdelta = 0;
                    if(discPolynomial.Coeffs8*transBdelta<0) nscdelta++;
                    if(transBdelta*transCdelta<0) nscdelta++;
                    if(transCdelta*transDdelta<0) nscdelta++;
                    if(transDdelta*transEdelta<0) nscdelta++;
                    if(transEdelta*transFdelta<0) nscdelta++;
                    if(transFdelta*transGdelta<0) nscdelta++;
                    if(transGdelta*transHdelta<0) nscdelta++;
                    if(transHdelta*transIdelta<0) nscdelta++;
                    ...
                    const double transDLB = (discPolynomial.Coeffs5 + 56*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs8 + 21*newdeltaLBsq*discPolynomial.Coeffs7
                                        + 6*newdeltaLB*discPolynomial.Coeffs6);
                    const double transELB = (discPolynomial.Coeffs4 + 70*newdeltaLBsq*newdeltaLBsq*discPolynomial.Coeffs8 + 35*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs7
                                        + 15*newdeltaLBsq*discPolynomial.Coeffs6 +5*newdeltaLB*discPolynomial.Coeffs5);
                    const double transFLB = (discPolynomial.Coeffs3 + 56*newdeltaLBsq*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs8 + 35*newdeltaLBsq*newdeltaLBsq*discPolynomial.Coeffs7
                                        + 20*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs6 + 10*newdeltaLBsq*discPolynomial.Coeffs5 + 4*newdeltaLB*discPolynomial.Coeffs4);
                    const double transGLB = (discPolynomial.Coeffs2 + 28*newdeltaLBsq*newdeltaLBsq*newdeltaLBsq*discPolynomial.Coeffs8 + 21*newdeltaLBsq*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs7
                                        + 15*newdeltaLBsq*newdeltaLBsq*discPolynomial.Coeffs6 + 10*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs5 + 6*newdeltaLBsq*discPolynomial.Coeffs4 + 3*newdeltaLB*discPolynomial.Coeffs3);
                    const double transHLB = (discPolynomial.Coeffs1 + 8*newdeltaLBsq*newdeltaLBsq*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs8 + 7*newdeltaLBsq*newdeltaLBsq*newdeltaLBsq*discPolynomial.Coeffs7
                                        + 6*newdeltaLBsq*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs6 + 5*newdeltaLBsq*newdeltaLBsq*discPolynomial.Coeffs5 + 4*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs4
                                        + 3*newdeltaLBsq*discPolynomial.Coeffs3 + 2*newdeltaLB*discPolynomial.Coeffs2);
                    const double transILB = (discPolynomial.Coeffs0 + newdeltaLBsq*newdeltaLBsq*newdeltaLBsq*newdeltaLBsq*discPolynomial.Coeffs8 + newdeltaLBsq*newdeltaLBsq*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs7
                                        + newdeltaLBsq*newdeltaLBsq*newdeltaLBsq*discPolynomial.Coeffs6 + newdeltaLBsq*newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs5 + newdeltaLBsq*newdeltaLBsq*discPolynomial.Coeffs4
                                        + newdeltaLBsq*newdeltaLB*discPolynomial.Coeffs3 + newdeltaLBsq*discPolynomial.Coeffs2 + newdeltaLB*discPolynomial.Coeffs1);
                    int nscLB = 0;
                    if(discPolynomial.Coeffs8*transBLB<0) nscLB++;
                    if(transBLB*transCLB<0) nscLB++;
                    if(transCLB*transDLB<0) nscLB++;
                    if(transDLB*transELB<0) nscLB++;
                    if(transELB*transFLB<0) nscLB++;
                    if(transFLB*transGLB<0) nscLB++;
                    if(transGLB*transHLB<0) nscLB++;
                    if(transHLB*transILB<0) nscLB++;
                    if(nscLB - nscdelta == 1) { //ie only one root between LB and UB
                        foundnewUB = true;
                    }
...
double RFRootFinder(double LB, double UB, DiscriminantCoeffs discCoeffs, double accuracy)
{
	...
	for(;l<maxIterationsRF; l++) {
            ...
            if(((fabs(adjustvalue) < accuracy) && (fabs((UB / LB) - 1)< 0.01)) || (l == maxIterationsRF)) { // if the result has met the desired precision and check it looks like a root! (or just way out of time!)
                break;
            }
...
//      Tighten running bounds on root
            if((y_x0 * y_UB) < 0) {
                LB = UB;
                y_LB = y_UB;
            } else if (y_x0 != 0) { // should always be true unless found exact root
